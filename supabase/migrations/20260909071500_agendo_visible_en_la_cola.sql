-- Las conversaciones agendadas vuelven a ser visibles en la cola.
--
-- EL PROBLEMA
--
-- Marcar "Agendó" dispara un trigger que pone derived_to_appointment = true en
-- la conversación, y esta vista excluía esas filas. El efecto buscado era
-- correcto —un lead ganado no debe seguir en la cola de trabajo— pero tenía una
-- consecuencia que nadie había notado: las conversiones se volvían INVISIBLES
-- para todo lo que lee esta vista.
--
-- Se detectó porque el contador "Agendó" que se agregó el 2026-09-08 mostraba
-- 0 de forma permanente, mientras el indicador "Agendados desde el módulo" —que
-- consulta la tabla directamente— mostraba 76. Dos números contradictorios en
-- la misma pantalla, y el equipo sí estaba agendando: 34 citas solo el 8 de
-- septiembre, 26 de ellas de una sola asesora.
--
-- El diagnóstico que se sacó de ese cero fue equivocado y llegó a un informe
-- para gerencia como "el punto ciego que hay que cerrar". No había tal: había
-- una vista que escondía justamente el dato que se estaba buscando.
--
-- LA CORRECCIÓN
--
-- Se quita el filtro por derived_to_appointment y se deja que el filtro por
-- ESTADO haga el trabajo. Ya existe y es más preciso: la pantalla arranca en
-- "Pendiente", así que las agendadas no ensucian la vista por defecto, pero
-- ahora se pueden contar y consultar.
--
-- Verificado antes de aplicar: de las 76 filas con derived_to_appointment,
-- las 76 tienen estado 'agendo' y ninguna carece de fila de estado. O sea que
-- ninguna va a aparecer como "pendiente" por error.
--
-- Efecto secundario deseable: sofia_cola_semanal() calculaba el ritmo semanal
-- sobre una base incompleta, porque las agendadas desaparecían del numerador y
-- del denominador a la vez. Ahora cuentan.
